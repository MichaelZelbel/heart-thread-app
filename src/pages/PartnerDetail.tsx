...
const PartnerDetail = () => {
  ...
  const [loveLanguages, setLoveLanguages] = useState<LoveLanguages>({
    physical: 3, words: 3, quality: 3, acts: 3, gifts: 3
  });
  ...
  const loadPartnerData = async () => {
    ...
    const { data, error } = await supabase.from("partners").select("*").eq("id", id).eq("user_id", session.user.id).single();
    if (error || !data) { toast.error("Partner not found"); navigate("/dashboard"); return; }
    setName(data.name);
    ...
    if (data.birthdate) {
      ...
      // Check if there's a Birthday moment and sync it
      const { data: birthdayMoment } = await supabase
        .from("moments")
        .select("moment_date")
        .eq("user_id", session.user.id)
        .contains("partner_ids", [id])
        .eq("event_type", "Birthday")
        .maybeSingle();
      if (birthdayMoment) {
        setBirthdate(new Date(birthdayMoment.moment_date));
        await supabase.from("partners").update({ birthdate: birthdayMoment.moment_date }).eq("id", id);
      } else {
        setBirthdate(null);
      }
    }
  ...
  const savePartnerData = useCallback(async (dataToSave: {
    name?: string; relationshipType?: string; genderIdentity?: string;
    country?: string; notes?: string; birthdate?: Date | null; loveLanguages?: LoveLanguages;
  }, showToast = false) => {
    if (!id) return;
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    ...
    if (dataToSave.name !== undefined) {
      if (!dataToSave.name.trim()) { toast.error("Partner name is required"); return; }
    ...
    const { error } = await supabase.from("partners").update(updateData).eq("id", id);
    if (error) { toast.error("Failed to save changes"); return; }

    // Sync birthdate to moments table
    if (dataToSave.birthdate !== undefined) {
  ...
  const debouncedSave = useCallback((dataToSave: any) => {
    ...
    saveTimeoutRef.current = setTimeout(() => { savePartnerData(dataToSave); }, 1000);
  }, [savePartnerData]);
  ...
  const handleArchive = async () => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    const { error } = await supabase.from("partners").update({ archived: true }).eq("id", id).eq("user_id", session.user.id);
  ...
  const handleDelete = async () => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    const { error } = await supabase.from("partners").delete().eq("id", id).eq("user_id", session.user.id);
    if (error) { toast.error("Failed to delete cherished"); return; }
      ...
      <nav className="bg-card border-b border-border shadow-soft">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={handleArchive} variant="outline"><Archive className="w-4 h-4 mr-2" />Archive</Button>
            <Button onClick={() => setShowDeleteDialog(true)} variant="outline"><Trash2 className="w-4 h-4 mr-2" />Delete</Button>
          </div>
        </div>
      </nav>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="conversation" className="relative">
              Conversation
              {!isPro && <span className="ml-1 text-[10px] font-medium text-primary bg-primary/10 px-1.5 py-0.5 rounded-full">Pro</span>}
            </TabsTrigger>
          ...
          <TabsContent value="conversation" className="space-y-6">
            {isPro ? <Card className="shadow-soft"><CardContent className="pt-6"><MessageCoach partnerId={id!} partnerName={name} initialContext={conversationContext} /></CardContent></Card> : <UpgradePrompt featureName="Conversation with Claire" description="Get personalized advice, message coaching, and relationship insights from Claire." />}
          </TabsContent>

          <TabsContent value="timeline" className="space-y-6">
            <Card className="shadow-soft">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Heart className="w-5 h-5 text-primary" />Your Story with {name}</CardTitle>
                <p className="text-sm text-muted-foreground">What happened before — tap any moment to ask Claire about it</p>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="documents" className="space-y-6">
            <Card className="shadow-soft">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5 text-primary" />Long-term Memory</CardTitle>
                <p className="text-sm text-muted-foreground">Your personal notes and reflections that help Claire understand your relationship</p>
              </CardHeader>
              <CardContent><CherishedDocuments partnerName={name} /></CardContent>
            </Card>
          </TabsContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete {name} and all associated data including moments, likes, and dislikes.
              <br /><br />
